...
public class MortgageDTOConverter {
	...
	public MortgageDTO getMortgageDTO(Mortgage mortgage) {
		...
		return new MortgageDTO(mortgage.getMortId(), mortgage.getFstName(), mortgage.getLstName(),
				mortgage.getEmailId(), mortgage.getPhone(), mortgage.getFamMembers(), mortgage.getChildren(),
				mortgage.getLuxury(), mortgage.getInSource(), mortgage.getMonthlyIn(), mortgage.getMonthlyLia(),
				mortgage.getZipCode());
	}
